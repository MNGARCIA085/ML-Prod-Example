...
class ModelTuner:
    def __init__(self, build_model_fn, log_dir="logs/tuning"):
        ...
        self.json_file = os.path.join(log_dir, f"{build_model_fn.__name__}_tuner_{timestamp}.json")
    ...
    def save_logs(self, model_name, best_hp, extra_params, history, val_metrics, test_metrics=None,
                  classification_metrics=None, elapsed_time=None):
        ...
        results = {
            ...
            "best_hyperparameters": {**best_hp.values, **extra_params},  # merged dict
            "environment": {
            ...
            #"val_metrics": {name: float(val_metrics[i]) for i, name in enumerate(self.metric_names)},
        }
    ...
    # run
    def run(self, train_ds, val_ds, test_ds=None, max_trials=5, executions_per_trial=1, epochs=20,
            patience=3, seed=42):
        # Reproducibility
        random.seed(seed)
        np.random.seed(seed)
        ...
        # Retrain best model to capture full history
        history_cb = HistoryCapture()
        best_model.fit(
            train_ds,
            validation_data=val_ds,
            epochs=epochs,
            callbacks=[early_stopping_cb, metrics_logger_cb, history_cb],
            verbose=0
        )
        ...
        # other relevant hyperarams
        trained_epochs = len(history_cb.history["loss"])
        #batch_size = getattr(train_ds, "_batch_size", None) or "unknown"
        ...
        if test_ds:
            ...
            y_pred = (y_pred_probs > 0.5).astype(int)  # binary threshold
